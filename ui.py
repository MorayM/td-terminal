...
class TodoistUI:
    ...
    def _get_visible_tasks(self) -> list[Task]:
        ...
        return [
            ...
            if filter_lower in t.project_name.lower()
        ]
    ...
    def _render_tasks(self, layout: dict):
        ...
        for i, task in enumerate(visible_tasks[self.scroll_offset:self.scroll_offset + visible_height]):
    ...
    def _render_task_line(self, y: int, num: int, task: Task, max_x: int, today: str):
        ...
        time_width = 6
        content_width = max_x - num_width - dot_width - project_width - labels_width - time_width - 4
        ...
        try:
            ...
            self.stdscr.addstr(y, num_width, "●", curses.color_pair(color) | curses.A_BOLD)
            ...
            self.stdscr.addstr(y, num_width + dot_width, content.ljust(content_width), content_attr)
            
            # Labels (abbreviated)
            x_pos = num_width + dot_width + content_width + 1
            if task.labels and labels_width > 0:
                labels_str = " ".join(f"@{l}" for l in task.labels[:2])
                if len(labels_str) > labels_width:
                    labels_str = labels_str[:labels_width - 2] + ".."
                self.stdscr.addstr(y, x_pos, labels_str.ljust(labels_width), curses.A_DIM)
            x_pos += labels_width
            
            # Due time
            if task.due_time:
                self.stdscr.addstr(y, x_pos, task.due_time.ljust(time_width), curses.A_DIM)
            x_pos += time_width
            ...
            self.stdscr.addstr(y, max_x - project_width - 1, project.rjust(project_width), curses.A_DIM)
